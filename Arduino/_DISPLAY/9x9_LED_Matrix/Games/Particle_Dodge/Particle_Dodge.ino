...
void setup() {
  ...
  randomSeed(analogRead(0)); // this gets a new random string to generate the level with.
...
void loop() {
  ...
  //this for loop is for the calm before the storm of particles.
  for(int i = 0; i < 25; i++)
  {
    yourguy(); // this function takes care of putting your guy into dispdata
    disp(1);
  }
  
  int cycle = 0;
  //this is where the particle doging gameplay happens
  while(1)
  {
    if (cycle == 15) cycle = 0; // reset cycle after 7
    
    yourguy();
    if(cycle == 0) particles();
    disp(1);
    cycle++;
  }
}
